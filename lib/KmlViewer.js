...
export default class KmlViewer extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (state.kmlText !== kmlText) {
      ...
      try {
        ...
        let data = parseFromString(kmlText); // Set state changes
    ...
    if (state.googleMap !== props.googleMap) stateChanges.googleMap = props.googleMap; // UPDATE MAP DRAWINGS?

    if (stateChanges.data !== undefined || stateChanges.googleMap !== undefined) {
      console.warn('UPDATE DRAWINGS REQUIRED'); // Remove old

      if (state.data) state.data.updateMapDrawing(null); // Create new
  ...
  onPlacemarkClick(obj) {
    ...
    obj.updateMapDrawing(this.state.googleMap); // Refresh ui
