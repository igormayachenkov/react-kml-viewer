...
exports.parseFromDOMDocument = function (domDocument) {
  ...
  if (!document) throw String('KML format error: Element "Document" is not found'); // Parce document as a container
  //   idBase = 0;
...
const placemarkChildParcers = {
  ...
  'Polygon': (parent, element) => {
    ...
    if (!obj.outerBoundaryIs) throw new ParsingError("Polygon with empty outerBoundaryIs"); // Add to the parent
...
const geometryChildParcers = {
  'outerBoundaryIs': (parent, element) => {
    ...
    if (!obj.linearRing) throw new ParsingError("outerBoundaryIs without LinearRing"); // Add to the parent
  ...
  'innerBoundaryIs': (parent, element) => {
    ...
    if (!obj.linearRing) throw new ParsingError("innerBoundaryIs without LinearRing"); // Add to the parent
  ...
  'LinearRing': (parent, element) => {
    ...
    if (!obj.coordinates) throw new ParsingError("LinearRing without cordinates"); // Add to the parent
